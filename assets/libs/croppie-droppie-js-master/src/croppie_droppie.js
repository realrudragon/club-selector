...
function croppie_droppie($container, $loader = null){
    ...
    var img_viewport = {
        width: parseInt($container.attr('img-width')) || 250,
        height: parseInt($container.attr('img-height')) || 250,
        type: $container.attr('img-type') || 'square'
    };

    var img_boundary = {
        width: $container.attr('box-width') || img_viewport.width + 25,
        height: $container.attr('box-height') || img_viewport.height + 25
    };
    ...
    function uploadImage(){
        if(!dataurl){
            ...
            // mo somkhane location . setTimeout
            setTimeout(function(){
              location.href = '';
            }, 2000);
            // mo somkhane location . setTimeout
    ...
    $cropped.css('margin', '0px 0px 18px 0px');
    $cropped.css('width', '-webkit-fill-available');
    // $cropped.css('width', 'auto');
    $cropped.css('height', 'auto');
    $cropped.css('border-style', 'dashed');
    $cropped.css('color', '#ccc');
    $cropped.css('border-color', '#ccc');
    $cropped.css('line-height', '200px');
    $cropped.css('text-align', 'center');
    $cropped.css('first_time', 'false');
}
